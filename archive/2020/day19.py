...
88: 97 72 | 10 52
22: 117 72 | 120 52
91: 72 72 | 52 72'''.split('\n')

messages = '''bbabbaabaabaaaababbbbabaabbaabab
...
aaabbbabaababbbabbbaaaabbabbbbaabaaabaaa'''.split('\n')
import re
...
def getRE(ruleNum):
    global rules
    rule = rules[str(ruleNum)]
    regex = ''
    if rule.__contains__('a') or rule.__contains__('b'):
        return rule[1]
    else:
        regex += '('
        rule = rule.split('|')
...
v = re.compile(getRE(0))
total = 0
for i in messages:
    if not(re.fullmatch(v,i) is None):
        total += 1
print(total)
