dat = '''LLLLL.LLLLLL.LLLLLLLLL.LLL.LL.LLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLLLLL.LLLLLLLLLL.LLLLLLL
...
LLLLL.LLLLLL.LLLLLLLLL.LLLLLL.LLLL.LLLLLLLLLLLLLLLLLLL.L.LL..LLLLLLLLLLL.LLLL.LLLLLLLLL.LLLLLLLL'''.split('\n')
    ...
    if dat[y][x] != '.':
        ...
        for X in range(x-1,x+1):
            for Y in range(y-1,y+1):
                if (not(X==x and Y==y)) and X>=0 and X<=len(dat[0]) and Y>=0 and Y<=len(dat) and dat[Y][X] == '#':
                    sC += 1
        if dat[y][x] == 'L':
            if sC == 0:
                return '#'
...
def runInstance(dat):
    newDat = []
    for yPos,yItem in enumerate(dat):
        appendNewDat = []
        for xPos in range(len(yItem)):
            appendNewDat.append(updateCell(xPos,yPos))
        newDat.append(appendNewDat)
    return newDat
prevDat = []
while True:
    prevDat = dat.copy()
