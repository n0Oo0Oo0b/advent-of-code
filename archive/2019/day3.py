...
runWire(wire1,0)
runWire(wire2,1)
c = t.Turtle() # Trial and error another turtle in the shell lol
'''

# Part 2
def getWire(data):
    loc = [0,0]
    # [ direction, offset, range(start,stop) , distance_2, start ]
    wire = []
    for i in data:
        f = i[0]
        m = int(i[1:])
        a = 1 if f=='U' or f=='D' else 0
        d = 1 if f=='U' or f=='R' else -1
        l = [ loc[a], loc[a]+(m*d) ]
        l2 = l.copy()
        l.sort()
        r = range(l[0],l[1])
        wire.append([ a, loc[a^1], r , l[1]-l[0], l2[0] ])
        loc[a] += m*d
    return wire
def checkOverlap(w1,w2):
    return w1[0]!=w2[0] and w1[2].__contains__(w2[1]) and w2[2].__contains__(w1[1])

wire1 = getWire(data[0])
wire2 = getWire(data[1])
...
for w1 in wire1:
    tL2 = 0
    for w2 in wire2:
        if checkOverlap(w1,w2):
            d = tL1+tL2+abs(w1[4]-w2[1])+abs(w2[4]-w1[1])
            if d>0:
